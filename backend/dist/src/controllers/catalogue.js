...
export const catalogueList = async (c) => {
    ...
    const products = await db.select({
        ...
        .where(and(eq(Product.is_active, true), searchQuery, featuredQuery))
        .orderBy(orderBy, orderByMap['latest'], asc(eq(Product.in_stock, 'empty')))
        .limit(limit)
        .offset(offset);
    return c.json({ products });
};
export const catalogueDetail = async (c) => {
    ...
    const products = await db.select({
        ...
        .from(Product)
        .innerJoin(Store, eq(Store.id, Product.store_id))
        .where(and(eq(Product.sku, sku), eq(Product.is_active, true)))
        .limit(1);
