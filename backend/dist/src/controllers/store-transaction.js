...
export const storeTransactionList = async (c) => {
    const store = c.get('store');
    const search = c.req.query('search') || '';
    const status = c.req.query('status') || '';
    const transactions = await db.select()
        .from(Transaction)
        .where(and(inArray(Transaction.status, ['in_process', 'sent', 'complained', 'completed', 'rejected']), eq(Transaction.store_id, store.id), search === '' ? undefined :
        or(ilike(Transaction.id, `%${search}%`), ilike(Transaction.description, `%${search}%`)), status === '' ? undefined :
        eq(Transaction.status, status)))
        .orderBy(desc(Transaction.created_at));
    return c.json({ transactions });
};
export const storeTransactionDetail = async (c) => {
    const store = c.get('store');
    ...
    const [transaction] = await db.select({
        ...
        .where(and(inArray(Transaction.status, ['in_process', 'sent', 'complained', 'completed', 'rejected']), eq(Transaction.id, Number(id)), eq(Transaction.store_id, store.id)))
        .limit(1);
    if (!transaction) {
...
export const storeTransactionStatusUpdate = async (c) => {
    const { id } = c.req.param();
    const { status, note } = await c.req.json();
    const store = c.get('store');
    const [transaction] = await db.select().from(Transaction)
        .where(and(eq(Transaction.id, Number(id)), eq(Transaction.store_id, store.id)))
        .limit(1);
    if (!transaction) {
        return c.json({ message: "Transaction not found" }, 404);
    }
