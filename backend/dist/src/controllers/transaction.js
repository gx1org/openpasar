...
export const transactionList = async (c) => {
    ...
    const transactions = await db.select().from(Transaction)
        .where(and(eq(Transaction.buyer_id, userId), searchQuery, status === '' ? undefined :
        eq(Transaction.status, status)))
        .orderBy(desc(Transaction.created_at));
    return c.json({ transactions });
};
export const transactionDetail = async (c) => {
    ...
    const [transaction] = await db.select({
        ...
        .where(and(eq(Transaction.id, Number(id)), eq(Transaction.buyer_id, userId)))
        .limit(1);
    if (!transaction) {
...
export const transactionStatusUpdate = async (c) => {
