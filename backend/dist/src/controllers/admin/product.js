...
export const adminProductList = async (c) => {
    ...
    const products = await db.select({
        ...
        .where(and(eq(Product.is_active, isActive), search === '' ? undefined :
        or(ilike(Product.name, `%${search}%`), ilike(Product.sku, `%${search}%`)), featuredQuery))
        .orderBy(orderBy, orderByMap['latest']);
    return c.json({ products });
};
export const adminProductDetail = async (c) => {
...
export const adminProductToggle = async (c) => {
...
export const adminProductFeatured = async (c) => {
    const { id } = c.req.param();
