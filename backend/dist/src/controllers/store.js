...
export const storeList = async (c) => {
...
export const storeCreate = async (c) => {
...
export const storeUpdate = async (c) => {
    ...
    const [check] = await db.select()
        .from(Store)
        .where(and(eq(Store.name, updatedStore.name), ne(Store.id, store.id)))
        .limit(1);
    if (check) {
        return c.json({ message: "Store name already exists" }, 400);
    }
...
export const storeDetail = async (c) => {
