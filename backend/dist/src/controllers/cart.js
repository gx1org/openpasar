...
export const removeFromCart = async (c) => {
    const userId = c.get('jwtPayload')?.id;
    const id = c.req.param("id");
    const [cart] = await db.select().from(Cart)
        .where(and(eq(Cart.user_id, userId), eq(Cart.id, Number(id))))
        .limit(1);
    if (!cart) {
...
export const checkout = async (c) => {
    ...
    const carts = await db.select({
        ...
        .from(Cart)
        .innerJoin(Product, eq(Cart.product_id, Product.id))
        .where(and(eq(Cart.user_id, userId), eq(Cart.store_id, store_id)));
    if (carts.length === 0) {
    ...
    await db.delete(Cart).where(and(eq(Cart.user_id, userId), eq(Cart.store_id, store_id)));
